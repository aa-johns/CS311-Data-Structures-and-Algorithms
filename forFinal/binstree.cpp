...
// NEW function!!!!
elem_t BST::PREorderSearch2(Vertex *V, elem_t K)
{
  if(V != NULL)
    {
      if(K == V->Elem)
	{
	  return K; // found the element in V                           
	}
      else if(K < V->Elem)
	{
	  //** traverse left sub-tree of V recursively                     
...
// NEW function!!!!
elem_t BST::find(string gname)
{
  elem_t found;
  found.name = gname;
  
  // if game is found in tree
  if(Search(found))
    {
      // go through tree and return item
      PREorderSearch2(Root,found);
    }
  else
    {
      elem_t blank;
